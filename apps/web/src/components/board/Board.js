...
function clientToSquare(clientX, clientY, boardRect, boardSize, flipped) {
...
export default function Board({ fen, orientation, legalMoves, lastMove, onMove, interactive = true, selectedSquare = null, onSquareClick, }) {
    ...
    const squareStyle = useCallback((sq) => {
    ...
    const isLegalTarget = useCallback((from, to) => {
    ...
    const handleMouseDown = (e, sq, piece) => {
    ...
    const pieceLayoutId = useCallback((square) => {
        if (lastMove?.to === square)
            return `piece-${lastMove.from}`;
        return `piece-${square}`;
    }, [lastMove]);
    const displayRanks = flipped ? [0, 1, 2, 3, 4, 5, 6, 7] : [7, 6, 5, 4, 3, 2, 1, 0];
    const displayFiles = flipped ? [7, 6, 5, 4, 3, 2, 1, 0] : [0, 1, 2, 3, 4, 5, 6, 7];
    return (_jsxs("div", { className: `relative inline-block${isShaking ? ' animate-board-shake' : ''}`, ref: boardRef, children: [_jsx(BoardBackground, { size: boardSize, flipped: flipped }), _jsx(SquareHighlight, { size: boardSize, flipped: flipped, selectedSquare: selectedSquare, legalSquares: legalTargetsForSelected, lastMove: lastMove, checkSquare: checkSquare }), _jsx("div", { className: "absolute inset-0", style: { width: boardSize, height: boardSize }, children: displayRanks.map((rankIdx) => displayFiles.map((fileIdx) => {
                    const sq = coordsToSquare(fileIdx, rankIdx);
                    return (_jsx("button", { type: "button", className: "absolute p-0 border-0 bg-transparent cursor-pointer", style: squareStyle(sq), onClick: () => handleSquareClick(sq), "aria-label": `Square ${sq}` }, sq));
                })) }), inCheck && checkSquare && (_jsx("div", { className: "absolute pointer-events-none z-[5]", style: squareStyle(checkSquare), children: _jsx(CheckRing, { squareSize: sqSize }) })), _jsx("div", { className: "absolute inset-0 pointer-events-none", style: { width: boardSize, height: boardSize }, children: _jsxs(LayoutGroup, { children: [pieces.map(({ square, piece }) => {
                            if (isDragging && square === dragFrom)
                                return null;
                            if (captureOverlay &&
                            ...
                            return (_jsx("div", { className: "absolute pointer-events-auto cursor-grab active:cursor-grabbing", style: squareStyle(square), onMouseDown: (e) => handleMouseDown(e, square, piece), children: _jsx(motion.div, { layoutId: pieceLayoutId(square), layout: "position", transition: { type: 'spring', stiffness: 500, damping: 40, duration: 0.12 }, children: _jsx(PieceComponent, { piece: piece, size: sqSize }) }) }, square));
                        }), captureOverlay && (_jsx("div", { className: "absolute pointer-events-none", style: squareStyle(captureOverlay.square), children: _jsx(PieceComponent, { piece: captureOverlay.piece, size: sqSize, isCapturing: captureOverlay.exiting }) }, `capture-${captureOverlay.square}`))] }) }), promotionPending && (_jsx(PromotionDialog, { square: promotionPending.to, color: promotionPending.color, boardSize: boardSize, flipped: flipped, onSelect: (p) => {
                    onMove(promotionPending.from, promotionPending.to, p);
                    setPromotionPending(null);
                }, onCancel: () => setPromotionPending(null) })), isDragging && dragPiece && (_jsx("div", { className: "fixed z-[60] pointer-events-none", style: {
                    left: ghostPos.x - sqSize / 2,
                    top: ghostPos.y - sqSize / 2,
                    width: sqSize,
                    height: sqSize,
                }, children: _jsx(PieceComponent, { piece: dragPiece, size: sqSize, isDragging: true }) }))] }));
}
